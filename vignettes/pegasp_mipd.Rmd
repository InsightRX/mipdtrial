...
```{r setup}
library(mipdtrial)
require(pkpegasparaginasemodifiedwurthwein, warn.conflicts = FALSE)
...
```{r}
# Pull demographic & body measure data
demo_data <- nhanes('DEMO_I', 2015)
bm_data <- nhanes('BMX_I', 2015)

# Filter for ages 0-17 (pediatric)
pediatric_data <- filter(demo_data, RIDAGEYR < 18) %>%
  merge(bm_data, by="SEQN") %>%
  select(SEQN,RIAGENDR,RIDAGEYR,BMXHT,BMXWT,BMXBMI)

# Rename variables for clarity
names(pediatric_data) <- c("ID", "Sex", "Age", "Height", "Weight", "BMI")
...
model_sim <- pkpegasparaginasemodifiedwurthwein::model()
parameters_sim <- pkpegasparaginasemodifiedwurthwein::parameters()
omega_sim <- pkpegasparaginasemodifiedwurthwein::omega_matrix()
ruv_sim <- pkpegasparaginasemodifiedwurthwein::ruv()
```
...
for (i in ids) {
  ...
  reg <- PKPDsim::new_regimen(
    amt = ifelse(covs$AGE$value >=22, # FDA/COG starting dose
                 2500 * covs$BSA$value, 
                 2000*covs$BSA$value), 
    interval = 14*24,
    n = 4,
  ...
  reg$dose_amts <-  ifelse(reg$dose_amts>3750, # cap doses at 3750 mg
                           3750,
                           reg$dose_amts) 
