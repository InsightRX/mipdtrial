...
We can also check how well our patients did at day 28.

```{r fig.width = 2, fig.height = 4}
res$eval_exposure |> 
  dplyr::filter(time == 672) |> 
  ggplot(aes(x = 1, y = conc)) +
  geom_rect(
    aes(
      xmin = -Inf, xmax = Inf, 
      ymin = target_design$min, ymax = target_design$max,
  ...
  labs(
    x = "",
    y = "Concentration at day 28 (IU/L)"
  ) +
