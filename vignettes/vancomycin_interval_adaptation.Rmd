...
tdm_scheme1 <- create_sampling_scheme(
...
dose_update_scheme <- create_regimen_update_scheme(
  anchor = c(3, 5),
  anchor_by = "dose",
  update_type = "dose"
)
...
for (i in dat$ID) {
  ...
  treatment_summary <- sample_and_adjust_by_dose(
    regimen_update_scheme = dose_update_scheme, 
    sampling_scheme = tdm_scheme1, # arm 1
    regimen = initial_reg,
    covariates = covs,
...
```{r}
interval_update_scheme <- create_regimen_update_scheme(
  anchor = c(3, 5),
  anchor_by = "dose",
...
for (i in dat$ID) {
  ...
  #################################################################################
  treatment_summary <- sample_and_adjust_by_dose(
    regimen_update_scheme = interval_update_scheme, 
    sampling_scheme = tdm_scheme1, # arm 1
    regimen = initial_reg,
    covariates = covs,
    pars_true_i = pars_true_i,
    ...
    interval_grid = c(6, 8, 12, 16, 24, 36),
    verbose = F
  )
