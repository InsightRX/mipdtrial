...
```{r setup}
library(mipdtrial)
require(pkbusulfanmccune, warn.conflicts = FALSE)
require(pkbusulfanshukla, warn.conflicts = FALSE)
...
for (i in ids) {
  ...
  sim_est <- PKPDsim::sim(
    model_est,
    parameters = treatment_summary$final_estimates, # best guess
    regimen = treatment_summary$final_regimen,
    covariates = covs,
    t_obs = auc_times,
...
Even with some model misspecification, target attainment (with 20% of target 
AUC) was still high:

```{r}
target_attainment <- results %>%
  mutate(
    ontarget = true_auc > 0.8 * target$value & true_auc < 1.2 * target$value
  ) %>%
