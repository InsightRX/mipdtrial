---
title: "Interval adaptation (infliximab)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Infliximab interval adaptation}
...
We will then use sample troughs, perform MAP estimations, and try to optimize
the dosing intervals to attain a target of 7.5 mg/L.
...
for (i in ids) {
  ...
  reg <- PKPDsim::new_regimen(
    amt = round(5 * covs$WT$value), # we will update this each "day" of the trial.
    interval = 8 * 7 * 24,
    n = 10,
    t_inf = 2,
    type = "infusion"
...
```{r}
ggplot(tdms) +
  geom_hline(yintercept = target_design$value, color = "red", linewidth = 1) +
